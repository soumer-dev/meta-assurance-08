  ...
  head: () => ({
    meta: [
      { title: "Obtenir un devis — Meta Assurances et Conseils" },
      {
        name: "description",
        content:
          "Devis gratuit et sans engagement en 2 minutes. Auto ou habitation : un expert vous contacte rapidement pour affiner votre couverture.",
      },
      { property: "og:title", content: "Votre devis personnalisé en 2 minutes" },
      { property: "og:description", content: "Devis gratuit, sans engagement. Réponse rapide d'un conseiller dédié." },
    ],
  }),
  component: DevisPage,
});
        ...
        <div className="relative mx-auto max-w-3xl px-5 lg:px-8">
          ...
          <div className="text-center">
            ...
            <h1 className="mt-5 font-display text-3xl font-semibold leading-[1.1] text-balance sm:text-4xl lg:text-5xl">
              ...
              <span className="italic text-sky">en 2 minutes</span>
            </h1>
            ...
            <div className="mt-8">
              <AnimatePresence mode="wait">
                {done ? (
                  ...
                  >
                    {step === 0 && <Step1 product={data.product} onSelect={(p) => update({ product: p })} />}
...
function Step1({ product, onSelect }: { product: Product | null; onSelect: (p: Product) => void }) {
  ...
  return (
    <div>
      ...
      <div className="mt-7 grid gap-4 sm:grid-cols-2">
        {opts.map(({ id, icon: Icon, title, sub }) => {
            ...
            >
              <div
                className={`inline-flex size-12 items-center justify-center rounded-xl ${
                  active ? "bg-sky text-navy" : "bg-sky/15 text-sky"
                }`}
              >
                <Icon className="size-6" />
              </div>
              <div>
                <div className={`text-lg font-semibold ${active ? "text-white" : "text-foreground"}`}>
                  {title}
                </div>
                <div className={`mt-1 text-sm ${active ? "text-white/75" : "text-muted-foreground"}`}>
                  {sub}
                </div>
              </div>
              {active && (
